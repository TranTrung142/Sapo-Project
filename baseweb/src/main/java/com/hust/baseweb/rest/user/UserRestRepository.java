...
public interface UserRestRepository extends PagingAndSortingRepository<DPerson, UUID>,
    ...
    @Query(
            "select p from DPerson p where p.type.id = :type and status.id = :status and concat(trim(p.person.firstName), trim(p.person.middleName), trim(p.person.lastName)) like %:fullNameString%")
